...
| INS_AssignLocalListNodePtrToHeapListNodePtr
    (localAddr heapAddr : nat) (next : CMachineInstruction)
| INS_AssignLocalListNodePtrToHeapListHead
    (localAddr heapAddr : nat) (next : CMachineInstruction)
| INS_AssignLocalListNodePtrToHeapListTail
    (localAddr heapAddr : nat) (next : CMachineInstruction)
...
| INS_AssignLocalListNodePtrToLocalListNodePtrNext
    (localAddr nodeAddr : nat) (next : CMachineInstruction)
...
| INS_SetListNodeNext (localAddr : nat) (nextAddr : Maybe nat)
    (next : CMachineInstruction)
| INS_SetListNodePrev (localAddr : nat) (prevAddr : Maybe nat)
    (next : CMachineInstruction)
| INS_SetLocalListHead
    (localAddr localNodeAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListTail
    (localAddr localNodeAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListNodeNext
    (localAddr localNodeAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListNodePrev
    (localAddr localNodeAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListHeadNull (localAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListTailNull (localAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListNodeNextNull (localAddr : nat) (next : CMachineInstruction)
| INS_SetLocalListNodePrevNull (localAddr : nat) (next : CMachineInstruction)
...
Notation "'@assignLocalListPtrToListPtrParameter' '(' offset ',' localAddr ')' ';' next" :=
    (INS_AssignLocalListPtrToListPtrParameter offset localAddr next)
    (in custom c_lang at level 3, offset constr, localAddr constr,
     next custom c_lang at level 200).
...
Notation "'@assignLocalListNodePtrToHeapListNodePtr' '(' localAddr ',' heapAddr ')' ';' next" :=
...
Notation "'@assignLocalListNodePtrToHeapListHead' '(' localAddr ',' heapAddr ')' ';' next" :=
...
Notation "'@assignLocalListNodePtrToLocalListPtrHead' '(' localAddr ',' listAddr ')' ';' next" :=
    (INS_AssignLocalListNodePtrToLocalListPtrHead localAddr listAddr next)
    (in custom c_lang at level 3, localAddr constr, listAddr constr,
     next custom c_lang at level 200).

Notation "'@assignLocalListNodePtrToLocalListPtrTail' '(' localAddr ',' listAddr ')' ';' next" :=
    (INS_AssignLocalListNodePtrToLocalListPtrTail localAddr listAddr next)
    (in custom c_lang at level 3, localAddr constr, listAddr constr,
     next custom c_lang at level 200).

Notation "'@assignLocalListNodePtrToHeapListNodeNext' '(' localAddr ',' heapAddr ')' ';' next" :=
    (INS_AssignLocalListNodePtrToHeapListNodeNext localAddr heapAddr next)
    (in custom c_lang at level 3, localAddr constr, heapAddr constr,
     next custom c_lang at level 200).

Notation "'@assignLocalListNodePtrToHeapListNodePrev' '(' localAddr ',' heapAddr ')' ';' next" :=
    (INS_AssignLocalListNodePtrToHeapListNodePrev localAddr heapAddr next)
    (in custom c_lang at level 3, localAddr constr, heapAddr constr,
     next custom c_lang at level 200).

Notation "'@assignLocalListNodePtrToLocalListNodePtrNext' '(' localAddr ',' nodeAddr ')' ';' next" :=
    (INS_AssignLocalListNodePtrToLocalListNodePtrNext localAddr nodeAddr next)
    (in custom c_lang at level 3, localAddr constr, nodeAddr constr,
     next custom c_lang at level 200).

Notation "'@assignLocalListNodePtrToLocalListNodePtrPrev' '(' localAddr ',' nodeAddr ')' ';' next" :=
    (INS_AssignLocalListNodePtrToLocalListNodePtrPrev localAddr nodeAddr next)
    (in custom c_lang at level 3, localAddr constr, nodeAddr constr,
     next custom c_lang at level 200).
...
Notation "'@setLinkedListHead' '(' localAddr ',' headAddr ')' ';' next" :=
...
Notation "'@setListNodePrev' '(' localAddr ',' prevAddr ')' ';' next" :=
    (INS_SetListNodePrev localAddr prevAddr next)
    (in custom c_lang at level 3, localAddr constr, prevAddr constr,
     next custom c_lang at level 200).

Notation "'@setLocalListHead' '(' localAddr ',' localNodeAddr ')' ';' next" :=
    (INS_SetLocalListHead localAddr localNodeAddr next)
    (in custom c_lang at level 3, localAddr constr, localNodeAddr constr,
     next custom c_lang at level 200).

Notation "'@setLocalListTail' '(' localAddr ',' localNodeAddr ')' ';' next" :=
    (INS_SetLocalListTail localAddr localNodeAddr next)
    (in custom c_lang at level 3, localAddr constr, localNodeAddr constr,
     next custom c_lang at level 200).

Notation "'@setLocalListNodeNext' '(' localAddr ',' localNodeAddr ')' ';' next" :=
    (INS_SetLocalListNodeNext localAddr localNodeAddr next)
    (in custom c_lang at level 3, localAddr constr, localNodeAddr constr,
     next custom c_lang at level 200).

Notation "'@setLocalListNodePrev' '(' localAddr ',' localNodeAddr ')' ';' next" :=
    (INS_SetLocalListNodePrev localAddr localNodeAddr next)
    (in custom c_lang at level 3, localAddr constr, localNodeAddr constr,
     next custom c_lang at level 200).

Notation "'@setLocalListHeadNull' '(' localAddr ')' ';' next" :=
    (INS_SetLocalListHeadNull localAddr next)
    (in custom c_lang at level 3, localAddr constr,
     next custom c_lang at level 200).

Notation "'@setLocalListTailNull' '(' localAddr ')' ';' next" :=
    (INS_SetLocalListTailNull localAddr next)
    (in custom c_lang at level 3, localAddr constr,
     next custom c_lang at level 200).
...
Notation "'@setLocalListNodePrevNull' '(' localAddr ')' ';' next" :=
    (INS_SetLocalListNodePrevNull localAddr next)
    (in custom c_lang at level 3, localAddr constr,
     next custom c_lang at level 200).
...
(* Set the head pointer for a local linked list pointer to the given address. *)
...
(* Set the tail pointer for a local linked list pointer to the given address. *)
Definition setLocalLinkedListPtrTail (addr : nat) (tailAddr : Maybe nat)
        : MachineM unit :=
    match tailAddr with
    | Nothing =>
        loadLocalLinkedListPtr addr ▶ fromJust ▶
        ...
        loadLinkedList paddr ▶
            λ '(List head _ count) ↦
                storeLinkedList paddr (List head Nothing count)
    | Just tailAddr' =>
        loadLocalLinkedListPtr addr ▶ fromJust ▶
        λ paddr ↦
        loadLinkedListNode tailAddr' »
        loadLinkedList paddr ▶
            λ '(List head _ count) ↦
                storeLinkedList paddr (List head tailAddr count)
    end.
...
(* Get the next pointer for a linked list node ptr. *)
Definition getLinkedListNodePtrNext (addr : nat) : MachineM (Maybe nat) :=
    loadLocalLinkedListPtr addr ▶ fromJust ▶
    loadLinkedListNode ▶
    λ '(Node next _ _) ↦
        ret next.
...
(* Get the prev pointer for a linked list node ptr. *)
Definition getLinkedListNodePtrPrev (addr : nat) : MachineM (Maybe nat) :=
    loadLocalLinkedListPtr addr ▶ fromJust ▶
    loadLinkedListNode ▶
    λ '(Node _ prev _) ↦
        ret prev.
...
(* Stub function for reading a LinkedListNodePtr parameter. *)
Parameter getLinkedListNodePtrParameter : nat → MachineM (Maybe nat).
...
Definition evalAssignLocalListNodePtrToListNodePtrParameter
        (offset : nat) (localAddr : nat) : MachineM unit :=
    getLinkedListNodePtrParameter offset ▶ fromJust ▶
    loadLinkedListNodePtr ▶
    storeLocalLinkedListNodePtr localAddr.
...
(* Evaluate a create local linked list node ptr instruction. *)
Definition evalCreateLocalLinkedListNodePtr (addr : nat) : MachineM unit :=
    createLocalLinkedListNodePtr Nothing ▶
    λ localAddr ↦
    if Nat.eqb localAddr addr then
        ret tt
    else
...
(* Evaluate an assignment instruction for a linked list node pointer. *)
Definition evalAssignLocalListNodePtrToHeapListNodePtr
        (localAddr heapAddr : nat) : MachineM unit :=
    loadLinkedListPtr heapAddr ▶
    storeLocalLinkedListPtr localAddr.

(* Evaluate an assignment instruction for a local linked list node pointer *)
(* to a list head. *)
Definition evalAssignLocalListNodePtrToListHead
        (localAddr listAddr : nat) : MachineM unit :=
    getLinkedListHead listAddr ▶
    storeLocalLinkedListPtr localAddr.

(* Evaluate an assignment instruction for a local linked list node pointer *)
(* to a list tail. *)
Definition evalAssignLocalListNodePtrToListTail
        (localAddr listAddr : nat) : MachineM unit :=
    getLinkedListTail listAddr ▶
    storeLocalLinkedListPtr localAddr.

(* Evaluate an assignment instruction for a local linked list node pointer *)
(* to a local list ptr head. *)
Definition evalAssignLocalListNodePtrToLocalListPtrHead
        (localAddr listAddr : nat) : MachineM unit :=
    getLocalLinkedListPtrHead listAddr ▶
    storeLocalLinkedListPtr localAddr.
...
(* Evaluate an assignment instruction for a local linked list node pointer to *)
(* a local list node pointer next. *)
Definition evalAssignLocalListNodePtrToLocalListNodePtrNext
        (localAddr nodeAddr : nat) : MachineM unit :=
    getLinkedListNodePtrNext nodeAddr ▶
    storeLocalLinkedListPtr localAddr.

(* Evaluate an assignment instruction for a local linked list node pointer to *)
(* a local list node pointer prev. *)
Definition evalAssignLocalListNodePtrToLocalListNodePtrPrev
        (localAddr nodeAddr : nat) : MachineM unit :=
    getLinkedListNodePtrPrev nodeAddr ▶
    storeLocalLinkedListPtr localAddr.
...
(* Evaluate a local set node next null instruction. *)
...
Definition evalLocalSetNodePrev (localAddr localNodeAddr : nat)
    ...
    λ addr ↦
    loadLocalLinkedListNodePtr localNodeAddr ▶
    setListNodePrev addr.

(* Evaluate a local set node prev null instruction. *)
Definition evalLocalSetNodePrevNull (localAddr : nat)
        : MachineM unit :=
    loadLocalLinkedListNodePtr localAddr ▶ fromJust ▶
    λ addr ↦
    setListNodePrev addr Nothing.

(* Evaluate a local set list head instruction. *)
Definition evalLocalSetListHead (localAddr localNodeAddr : nat)
        : MachineM unit :=
    loadLocalLinkedListPtr localAddr ▶ fromJust ▶
    λ addr ↦
    loadLocalLinkedListNodePtr localNodeAddr ▶
...
Fixpoint eval (ins : CMachineInstruction) : MachineM CStatusCode :=
    ...
    | INS_AssignLocalListNodePtrToListNodePtrParameter
        ...
        evalAssignLocalListNodePtrToListNodePtrParameter heapAddr localAddr »
        eval next
    | INS_AssignLocalListPtrToListPtrParameter
            offset localAddr next =>
        evalAssignLocalListPtrToListPtrParameter offset localAddr »
        eval next
    | INS_AssignLocalListHeapPtrToLocalListPtr localHeapAddr localAddr next =>
        evalAssignLocalListHeapPointerToLocalListPtr localHeapAddr localAddr »
        eval next
    | INS_AssignLocalListNodePtrToHeapListNodePtr localAddr heapAddr next =>
        evalAssignLocalListNodePtrToHeapListNodePtr localAddr heapAddr »
        eval next
    | INS_AssignLocalListNodePtrToHeapListHead localAddr heapAddr next =>
        evalAssignLocalListNodePtrToListHead localAddr heapAddr »
        eval next
    | INS_AssignLocalListNodePtrToHeapListTail localAddr heapAddr next =>
        evalAssignLocalListNodePtrToListTail localAddr heapAddr »
        eval next
    | INS_AssignLocalListNodePtrToLocalListPtrHead localAddr listAddr next =>
    ...
    | INS_AssignLocalListNodePtrToHeapListNodeNext localAddr heapAddr next =>
        ...
        eval next
    | INS_AssignLocalListNodePtrToHeapListNodePrev localAddr heapAddr next =>
        evalAssignLocalListNodePtrToListNodePrev localAddr heapAddr »
        eval next
    | INS_AssignLocalListNodePtrToLocalListNodePtrNext
            localAddr nodeAddr next =>
        evalAssignLocalListNodePtrToLocalListNodePtrNext localAddr nodeAddr »
        eval next
    | INS_AssignLocalListNodePtrToLocalListNodePtrPrev
            localAddr nodeAddr next =>
        evalAssignLocalListNodePtrToLocalListNodePtrPrev localAddr nodeAddr »
        eval next
    | INS_IncrementListCount localAddr next =>
    ...
    | INS_SetListNodeNext localAddr nextAddr next =>
        ...
        eval next
    | INS_SetListNodePrev localAddr prevAddr next =>
        evalSetNodePrev localAddr prevAddr »
        eval next
    | INS_SetLocalListHead localAddr localNodeAddr next =>
        evalLocalSetListHead localAddr localNodeAddr »
        eval next
    | INS_SetLocalListTail localAddr localNodeAddr next =>
        evalLocalSetListTail localAddr localNodeAddr »
        eval next
    | INS_SetLocalListNodeNext localAddr localNodeAddr next =>
        evalLocalSetNodeNext localAddr localNodeAddr »
        eval next
    | INS_SetLocalListNodePrev localAddr localNodeAddr next =>
        evalLocalSetNodePrev localAddr localNodeAddr »
        eval next
    | INS_SetLocalListHeadNull localAddr next =>
        evalLocalSetListHeadNull localAddr »
        eval next
    | INS_SetLocalListTailNull localAddr next =>
...
Definition insListAppendTail : CMachineInstruction :=
    [[
        @createLocalLinkedListPtr(1);
        @createLocalLinkedListNodePtr(2);
        @createLocalLinkedListNodePtr(3);
        ...
        @assignLocalListPtrToListPtrParameter(1,1);
        @assignLocalListNodePtrToListNodePtrParameter(2,2);
        @assignLocalListNodePtrToLocalListPtrTail(3,1);
        @setLocalListNodePrev(2,3);
        @setLocalListNodeNextNull(2);
        @incrementCount(1);
        @if (@isListNodePtrPreset(3)) {
            @setLocalListTail(1,2);
            @assignLocalListNodePtrToLocalListNodePtrPrev(4,2);
            @setLocalListNodeNext(4,2);
            @return StatusSuccess;
        } @else {
            @setLocalListHead(1,2);
            @setLocalListTail(1,2);
