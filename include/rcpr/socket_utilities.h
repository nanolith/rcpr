...
#define __INTERNAL_RCPR_IMPORT_socket_utilities_sym(sym) \
    ...
    sym ## socket_utility_move_descriptors(int x, int* y, ...) { \
            ...
            return retval; } \
    static inline status FN_DECL_MUST_CHECK \
    sym ## socket_utility_setfds(int x, int y, ...) { \
            status retval; \
            va_list args; \
            va_start(args, y); \
            retval = RCPR_SYM(vsocket_utility_setfds)(x, y, args); \
            va_end(args); \
            return retval; } \
    static inline status FN_DECL_MUST_CHECK \
    sym ## socket_utility_close_other_fds(int x) { \
            return RCPR_SYM(socket_utility_close_other_fds)(x); } \
