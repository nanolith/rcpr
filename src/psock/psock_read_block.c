    ...
    RCPR_SYM(psock)* sock)
{
    ...
    while (!done)
    {
        /* verify that there is not currently a fiber blocked for reading. */
        if (s->read_block_fib != NULL)
        {
            return ERROR_PSOCK_DOUBLE_BLOCK_UNSUPPORTED;
        }

        /* set this fiber as the read blocked fiber. */
        s->read_block_fib = current_fib;
