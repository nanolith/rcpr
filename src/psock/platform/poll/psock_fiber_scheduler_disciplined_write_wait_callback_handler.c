/**
 * \file select/psock_fiber_scheduler_disciplined_write_wait_callback_handler.c
 *
 * \brief Handle a write wait callback via select.
 *
    ...
    void* context, fiber* yield_fib, int yield_event, void* yield_param)
{
    ...
    ctx->poll_events[idx].events = POLLOUT;
