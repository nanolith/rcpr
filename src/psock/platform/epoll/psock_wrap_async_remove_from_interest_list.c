/**
 * \file epoll/psock_wrap_async_remove_from_interest_list.c
 *
    ...
    RCPR_SYM(psock_wrap_async)* sock)
{
    ...
    /* remove this event from the interest list. */
    event.events = 0;
    event.data.ptr = NULL;
    retval = epoll_ctl(ctx->ep, EPOLL_CTL_DEL, pchild->descriptor, &event);
    if (retval < 0)
    {
