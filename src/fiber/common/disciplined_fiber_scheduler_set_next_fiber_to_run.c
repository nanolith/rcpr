/**
 * \file fiber/common/disciplined_fiber_scheduler_set_next_fiber_to_run.c
 *
 * \brief Add a fiber to the run queue, making it the next fiber to run.
 *
    ...
    const rcpr_uuid* resume_id, int resume_event, void* resume_param)
{
    ...
    /* add this fiber to the start of the run queue. */
    return queue_insert(ctx->run_queue, fiber_resource_handle(fib));
}
