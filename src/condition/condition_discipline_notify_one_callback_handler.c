/**
 * \file condition/condition_discipline_notify_one_callback_handler.c
 *
 * \brief Handle a conditional notify one request.
 *
    ...
    void* yield_param)
{
    ...
    /* are there waiting fibers? */
    if (slist_count(cond->wait_list) > 0)
    {
        /* get a waiting fiber. */
        fiber* fib;
