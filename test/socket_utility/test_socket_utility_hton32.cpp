/**
 * \file test/test_socket_utility_hton32.cpp
 *
...
TEST_SUITE(socket_utility_hton32);

/**
 * Verify that hton32 works.
 */
TEST(basics)
{
    ...
    TEST_EXPECT(
        EXPECTED_NETWORK_VALUE ==
            socket_utility_hton32(EXPECTED_NETWORK_VALUE));
    ...
    TEST_EXPECT(
        EXPECTED_NETWORK_VALUE ==
            socket_utility_hton32(EXPECTED_HOST_VALUE));

    /* doing the conversion twice undoes it. */
    TEST_EXPECT(
        EXPECTED_HOST_VALUE ==
            socket_utility_hton32(
                socket_utility_hton32(EXPECTED_HOST_VALUE)));
#endif
}
