/**
 * \file test/test_psock_read_write_boxed_uint8.cpp
 *
 * \brief Unit tests for psock_read_boxed_uint8 / psock_write_boxed_uint8.
 */
...
TEST_SUITE(psock_read_write_boxed_uint8);

/**
 * Verify that we can read and write boxed uint8 values.
 */
TEST(read_write)
{
    ...
    int lhs;
    int rhs;
    const uint8_t EXPECTED_VALUE = 39;
    uint8_t val = 0;
    ...
    TEST_ASSERT(
        STATUS_SUCCESS ==
            psock_write_boxed_uint8(ls, EXPECTED_VALUE));

    /* we should be able to read a 64-bit integer from the rhs socket. */
    TEST_ASSERT(
        STATUS_SUCCESS ==
            psock_read_boxed_uint8(rs, &val));
...
TEST(write_invalid_read_error)
{
    ...
    /* reading a boxed uint8 value should fail. */
    TEST_ASSERT(
