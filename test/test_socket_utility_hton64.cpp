/**
 * \file test/test_socket_utility_hton64.cpp
 *
...
TEST_SUITE(socket_utility_hton64);

/**
 * Verify that hton64 works.
 */
TEST(basics)
{
    ...
    TEST_EXPECT(
        EXPECTED_NETWORK_VALUE ==
            socket_utility_hton64(EXPECTED_NETWORK_VALUE));
    ...
    TEST_EXPECT(
        EXPECTED_NETWORK_VALUE ==
            socket_utility_hton64(EXPECTED_HOST_VALUE));

    /* doing the conversion twice undoes it. */
    TEST_EXPECT(
        EXPECTED_HOST_VALUE ==
            socket_utility_hton64(
                socket_utility_hton64(EXPECTED_HOST_VALUE)));
#endif
}
