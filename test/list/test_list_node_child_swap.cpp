/**
 * \file test/list/test_list_node_child_swap.cpp
 *
 * \brief Unit tests for list_node_child_swap.
 */
...
TEST_SUITE(list_node_child_swap);

/**
 * Verify that we can swap the child resource in a list_node.
 */
TEST(basics)
{
    ...
    TEST_ASSERT(
        STATUS_SUCCESS ==
            list_append_tail(l, other_resource));
    ...
    TEST_ASSERT(
        STATUS_SUCCESS ==
            list_node_child_swap(head, &newother_resource));
