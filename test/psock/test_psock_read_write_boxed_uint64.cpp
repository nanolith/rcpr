/**
 * \file test/test_psock_read_write_boxed_uint64.cpp
 *
 * \brief Unit tests for psock_read_boxed_uint64 / psock_write_boxed_uint64.
 */
...
TEST_SUITE(psock_read_write_boxed_uint64);

/**
 * Verify that we can read and write boxed uint64 values.
 */
TEST(read_write)
{
    ...
    TEST_ASSERT(
        STATUS_SUCCESS ==
            psock_write_boxed_uint64(ls, EXPECTED_VALUE));

    /* we should be able to read a 64-bit integer from the rhs socket. */
    TEST_ASSERT(
        STATUS_SUCCESS ==
            psock_read_boxed_uint64(rs, &val));
...
TEST(write_invalid_read_error)
{
    ...
    /* reading a boxed uint64 value should fail. */
    TEST_ASSERT(
