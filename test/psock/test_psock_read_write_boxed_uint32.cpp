/**
 * \file test/test_psock_read_write_boxed_uint32.cpp
 *
 * \brief Unit tests for psock_read_boxed_uint32 / psock_write_boxed_uint32.
 */
...
TEST_SUITE(psock_read_write_boxed_uint32);

/**
 * Verify that we can read and write boxed uint32 values.
 */
TEST(read_write)
{
    ...
    int lhs;
    int rhs;
    const uint32_t EXPECTED_VALUE = 128929839;
    uint32_t val = 0;
    ...
    TEST_ASSERT(
        STATUS_SUCCESS ==
            psock_write_boxed_uint32(ls, EXPECTED_VALUE));

    /* we should be able to read a 64-bit integer from the rhs socket. */
    TEST_ASSERT(
        STATUS_SUCCESS ==
            psock_read_boxed_uint32(rs, &val));
...
TEST(write_invalid_read_error)
{
    ...
    /* reading a boxed uint32 value should fail. */
    TEST_ASSERT(
