...
TEST_SUITE(psock_fiber_read_write_dual);
...
static status test_fibers_create(
    RCPR_SYM(fiber)** read_fib, RCPR_SYM(fiber)** write_fib,
    RCPR_SYM(allocator)* alloc, main_context* mctx, int sock,
    status (*read_fiber_entry)(void*), status (*write_fiber_entry)(void*));
static status test_fiber_context_resource_release(RCPR_SYM(resource)* r);
static status fiber_manager_entry(void* vctx);
static status client_read_fiber_entry(void* vctx);
static status client_write_fiber_entry(void* vctx);
static status server_read_fiber_entry(void* vctx);
static status server_write_fiber_entry(void* vctx);
...
TEST(dual_basics)
{
    ...
    fiber* tmp_read_fiber = nullptr;
    fiber* tmp_write_fiber = nullptr;
    ...
    for (int i = 0; i < 50; ++i)
    {
        ...
        TEST_ASSERT(
            STATUS_SUCCESS ==
                test_fibers_create(
                    &tmp_read_fiber, &tmp_write_fiber, alloc, ctx, left,
                    &server_read_fiber_entry, &server_write_fiber_entry));

        /* add these fibers to the scheduler. */
        TEST_ASSERT(
            STATUS_SUCCESS == fiber_scheduler_add(sched, tmp_read_fiber));
        TEST_ASSERT(
            STATUS_SUCCESS == fiber_scheduler_add(sched, tmp_write_fiber));

        /* create the server test context. */
        TEST_ASSERT(
            STATUS_SUCCESS ==
                test_fibers_create(
                    &tmp_read_fiber, &tmp_write_fiber, alloc, ctx, right,
                    &client_read_fiber_entry, &client_write_fiber_entry));
...
/**
 * \brief Create two test fibers.
 *
 * \param read_fib          Pointer to receive the read fiber intance pointer.
 * \param write_fib         Pointer to receive the write fiber intance pointer.
 * \param alloc             The allocator to use for this operation.
 * \param mctx              Main context.
 * \param sock              The socket to use for this operation.
 * \param read_fiber_entry  The entry point for the read fiber.
 * \param write_fiber_entry The entry point for the write fiber.
 *
    ...
    status (*read_fiber_entry)(void*), status (*write_fiber_entry)(void*))
{
    ...
    fiber* read_inst = NULL;
    fiber* write_inst = NULL;
    psock* ps = NULL;
    ...
    /* create the read fiber instance. */
    retval =
        fiber_create(
            &read_inst, alloc, mctx->sched, 1024 * 1024, tmp, read_fiber_entry);
    if (STATUS_SUCCESS != retval)
    {
        goto cleanup_tmp;
    }

    /* create the write fiber instance. */
    retval =
        fiber_create(
            &write_inst, alloc, mctx->sched, 1024 * 1024, tmp,
            write_fiber_entry);
    if (STATUS_SUCCESS != retval)
    {
        goto cleanup_read_inst;
    }
    ...
    if (STATUS_SUCCESS != retval)
    {
        goto cleanup_write_inst;
    }

    /* wrap this psock instance as an async socket. */
    retval = psock_create_wrap_async(&tmp->sock, alloc, read_inst, ps);
    if (STATUS_SUCCESS != retval)
    {
        goto cleanup_ps;
    }
...
cleanup_write_inst:
    if (NULL != write_inst)
    {
        release_retval = resource_release(fiber_resource_handle(write_inst));
        if (STATUS_SUCCESS != release_retval)
        {
            retval = release_retval;
        }
...
cleanup_read_inst:
    if (NULL != read_inst)
    {
        release_retval = resource_release(fiber_resource_handle(read_inst));
        if (STATUS_SUCCESS != release_retval)
        {
            retval = release_retval;
        }
...
static status test_fiber_context_resource_release(RCPR_SYM(resource)* r)
{
    ...
    /* decrement the reference count, and return if it isn't zero. */
    ctx->ref_count -= 1;
    if (ctx->ref_count > 0)
    {
...
/**
 * \brief Entry point for the client read test fiber.
 *
...
/**
 * \brief Entry point for the client write test fiber.
 *
 * This fiber sends a request to the server test fiber and reads a response.
 *
 * \param vctx          The type erased test context.
...
/**
 * \brief Entry point for the server read test fiber.
 *
 * This fiber sends a request to the server test fiber and reads a response.
 *
 * \param vctx          The type erased test context.
...
/**
 * \brief Entry point for the server write test fiber.
 *
 * This fiber sends a request to the server test fiber and reads a response.
 *
 * \param vctx          The type erased test context.
