/**
 * \file test/fiber/test_fiber_scheduler_add.cpp
 *
 * \brief Unit tests for fiber_scheduler_add.
 */
...
TEST_SUITE(fiber_scheduler_add);
...
static status test_fiber_scheduler_callback(
    ...
    fiber** resume_fib, const rcpr_uuid** resume_id,
...
/**
 * Verify that we can add a fiber to the scheduler instance.
 */
TEST(add)
{
    ...
    /* Add PRECONDITIONS: zero out the calls. */
    ctx.calls = 0;

    /* add the fiber dummy fiber instance to the scheduler instance. */
    TEST_ASSERT(STATUS_SUCCESS == fiber_scheduler_add(sched, dummy));

    /* add POSTCONDITIONS: the scheduler should have been called. */
    TEST_EXPECT(1 == ctx.calls);
    TEST_EXPECT(nullptr != ctx.yield_fiber1);
    TEST_EXPECT(FIBER_SCHEDULER_YIELD_EVENT_ADD_FIBER == ctx.yield_event1);
    TEST_EXPECT(dummy == ctx.yield_param1);
